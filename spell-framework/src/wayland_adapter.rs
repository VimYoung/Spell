...
use crate::{
    ...
    wayland_adapter::{
        common::PointerState, fractional_scaling::delegate_fractional_scale,
        viewporter::delegate_viewporter,
    },
};
...
impl SpellLock {
    ...
    fn unlock_finger(&mut self, error_callback: Box<dyn FnOnce() + Send>) {
        ...
        std::thread::spawn(move || {
            ...
            fn unlock_internal(sender: Sender<bool>) -> PamResult<()> {
                let finger = way_helper::FingerprintInfo;
    ...
    fn unlock(
        ...
        let user_pass = way_helper::UsernamePassConvo {
